#!/usr/bin/env python3
# Seed script - clears all tables and adds fresh example data.
# Run with: pipenv run python server/seed.py
...
with app.app_context():
    ...
    leg_day = Workout(
        date=date(2026, 7, 21),
        duration_minutes=60,
        notes="Heavy leg day, focus on form.",
    )
    cardio_session = Workout(
        date=date(2026, 7, 22),
        duration_minutes=45,
        notes="Steady state cardio and core work.",
    )
    full_body = Workout(
        date=date(2026, 7, 24),
        duration_minutes=75,
        notes="Full body circuit, 3 rounds.",
    )
